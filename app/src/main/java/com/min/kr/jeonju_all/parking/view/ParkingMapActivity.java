...
public class ParkingMapActivity extends NMapActivity implements OnMapStateChangeListener,OnMapViewTouchEventListener,
    ...
    public View onCreateCalloutOverlayView(NMapOverlay nMapOverlay, NMapOverlayItem nMapOverlayItem, Rect rect) {
        ...
        for (int i=0;i<datas.size();i++){
            if (clickStore.equals(datas.get(i).getTitle())){
                ...
                tel = datas.get(i).getTel()+" / "+datas.get(i).getType()+"("+datas.get(i).getParkingFee()+")";
                type = datas.get(i).getMemo();
        ...
        return new NMapCalloutCustomOverlayView(ParkingMapActivity.this, nMapOverlay, nMapOverlayItem, rect, name, address, tel, type, ceoName, x, y, 40);
    }
        ...
        if (nMapMyLocationOverlay != null) {
            ...
            if (nMapLocationManager.isMyLocationEnabled()) {
                ...
                if (!isMyLocationEnabled) {
                    ...
                    Toast.makeText(ParkingMapActivity.this,"현재위치를 찾고 있습니다...",Toast.LENGTH_LONG).show();
                    locationType = 1;
                }
            }
        }
