...
import com.example.minkr.jeonju_all.facility.data.HospitalDatas;
...
public class MedicineMapActivity extends NMapActivity implements OnMapStateChangeListener,OnMapViewTouchEventListener,
    ...
    public View onCreateCalloutOverlayView(NMapOverlay nMapOverlay, NMapOverlayItem nMapOverlayItem, Rect rect) {
        ...
        for (int i=0;i<datas.size();i++){
            if (clickStore.equals(datas.get(i).getMediName())){
                ...
                time = datas.get(i).getMediConDate()+"("+datas.get(i).getMediStime()+" ~ "+datas.get(i).getMediEtime()+")";
        ...
        return new NMapCalloutCustomOverlayView(MedicineMapActivity.this, nMapOverlay, nMapOverlayItem, rect, name, address, tel, time, ceoName, x, y, 20);
    }
        ...
        if (nMapMyLocationOverlay != null) {
            ...
            if (nMapLocationManager.isMyLocationEnabled()) {
                ...
                if (!isMyLocationEnabled) {
                    ...
                    Toast.makeText(MedicineMapActivity.this,"현재위치를 찾고 있습니다...",Toast.LENGTH_LONG).show();
                    locationType = 1;
                }
            }
        }
